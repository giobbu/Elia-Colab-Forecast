...
def calculate_rmse(df, pred_col, targ_col='targets'):
    " Calculate RMSE Loss."
...
def calculate_mae(df, pred_col, targ_col='targets'):
    " Calculate MAE Loss."
    assert pred_col in df.columns, "prediction column is missing"
    assert targ_col in df.columns, "target column is missing"
...
def calculate_pinball_losses(df, confidence_10_col, confidence_90_col, targ_col='targets'):
